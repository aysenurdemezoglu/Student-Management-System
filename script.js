...
`;
    ...
    document
      .getElementById("courseSelectHome")
      .addEventListener("change", function () {
        getFailedStudentsByCourse();
      });
    document
      .getElementById("courseSelectHome2")
      .addEventListener("change", function () {
        getSuccededStudentsByCourse();
      });
    document
      .getElementById("selectStudentHome")
      .addEventListener("change", function () {
...
document
  ...
  .addEventListener("click", function () {
    ...
    if (
      isNaN(studentGrade1Input.value) ||
      parseInt(studentGrade1Input.value) < 0 ||
      parseInt(studentGrade1Input.value) > 100
    ) {
      alert(
        "Please enter a valid numeric value between 0 and 100 for Midterm Grade."
      );
      return;
    }
    if (
      isNaN(studentGrade2Input.value) ||
      parseInt(studentGrade2Input.value) < 0 ||
      parseInt(studentGrade2Input.value) > 100
    ) {
      alert(
        "Please enter a valid numeric value between 0 and 100 for Final Grade."
      );
      return;
    }
    ...
    if (existingStudent) {
      ...
      alert(
        "Student with ID " +
          studentIdInput.value +
          " already exists. Form filled with existing data."
      );
      return;
    }
    ...
    students.forEach(function (std) {
      if (std.id === studentIdInput.value) {
        var student = students.find(
          (student) => student.id === studentIdInput.value
        );
        if (student.lectures === studentLecturesInput.value) {
          ...
          alert(
            "Student with ID " +
              studentIdInput.value +
              " has already selected the course " +
              selectedCourseId +
              "."
          );
          return;
...
document
  .getElementById("addLectureButton")
  .addEventListener("click", function () {
    ...
    if (
      isNaN(lectureCreditInput.value) ||
      parseInt(lectureCreditInput.value) <= 0
    ) {
...
function removeLectureFromTable(code) {
  var lectureList = document.getElementById("lectureList");
  var rowToRemove = Array.from(lectureList.children).find(
    (row) => row.firstChild.textContent === code
  );
  if (rowToRemove) {
...
function addLetterToTable(student, letterGrade) {
  var studentList = document.getElementById("studentList");
  var studentInfoHTML =
    student.getStudentInfoHTML() + `<td>${letterGrade}</td>`;
  var listItem = document.createElement("tr");
  listItem.innerHTML = studentInfoHTML;
  studentList.appendChild(listItem);
}
...
function searchStudentsByLecture() {
  var searchLectureInput = document
    .getElementById("searchLecture")
    .value.toLowerCase();
...
function getFailedStudentsByCourse() {
  ...
  var lecturesFromStorage = JSON.parse(localStorage.getItem("lectures")) || [];
  var studentsFromStorage = JSON.parse(localStorage.getItem("students")) || [];

  console.log("Selected Source Code:", selectedCourseCode);
  console.log("All Lectures:", lecturesFromStorage);
  console.log("All Students:", studentsFromStorage);

  var selectedLecture = lecturesFromStorage.find(
    (l) => l.code === selectedCourseCode
  );
  if (!selectedLecture) {
  ...
  console.log("Name of the selected course:", selectedLecture.name);

  var failedStudents = studentsFromStorage.filter(
    (student) =>
      student.lectures === selectedLecture.name && student.letterGrade === "F"
  );

  console.log("Failed students found:", failedStudents);
...
function displayFailedStudents(failedStudents) {
  ...
  failedStudents.forEach((student) => {
    let key = `${student.id}-${student.lectures}`;
    if (!addedSet.has(key)) {
...
function getSuccededStudentsByCourse() {
  ...
  var selectedLectureName = selectedLecture.name;

  var passedStudents = studentsFromStorage.filter(
    (student) =>
      student.lectures === selectedLectureName && student.letterGrade !== "F"
  );

  console.log(
    "Passed students for " + selectedLectureName + ":",
    passedStudents
  );

  displaySuccededStudents(passedStudents);

  console.log("Seçilen Kod:", selectedCourseCode);
  console.log("Eşleşen Ders Adı:", selectedLectureName);
  console.log("Geçen Öğrenciler:", passedStudents);
}
...
function displayStudentsGpa(studentID) {
  ...
  var studentRecords = studentsFromStorage.filter((s) => s.id === studentID);

  var totalCredits = 0;
  var totalGradePoints = 0;

  studentRecords.forEach((record) => {
    var lecture = lecturesFromStorage.find((l) => l.name === record.lectures);
    if (lecture) {
      var credit = parseInt(lecture.credit);
      var gpaPoint = convertLetterToGpa(record.letterGrade);
      totalCredits += credit;
      totalGradePoints += gpaPoint * credit;
    }
  });

  if (totalCredits === 0) {
    studentListGpa.innerHTML = `<p>No courses found for GPA calculation.</p>`;
    return;
  }
  ...
  var listItem = document.createElement("div");
  listItem.innerHTML = `<p>${student.name} ${
    student.surname
  } - GPA: ${gpa.toFixed(2)}</p>`;
...
function updateStudent(studentId, lectures) {
  var studentToUpdate = students.find(
    (student) => student.id === studentId && student.lectures === lectures
  );
  if (!studentToUpdate) {
...
function updateLecture(lectureCode) {
  var lectureToUpdate = lectures.find(
    (lecture) => lecture.code === lectureCode
  );
  if (!lectureToUpdate) {
...
function confirmUpdateForLecture(lectureCode) {
  ...
  var updatedLectureCredit = document.getElementById(
    "updateLectureCredit"
  ).value;
